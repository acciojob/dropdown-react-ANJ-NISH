...
function App() 
{
    ...
    const [citytaken, setCitytaken]=useState();
	const [landmarktaken, setLandmarktaken]=useState();
	const [citiesObj, setCitiesObj]=useState();
	const [landObj, setLandobj]=useState();

	

	function fixCities(e)
	{

	  let apnastate=states.filter((indistate, index)=> {if(index===parseInt(e.target.value)) {return indistate}});
      let cityarr;

	  cityarr=apnastate[0].city;

	  setCitiesObj(cityarr);

	  let citiesarr=apnastate[0].city.map((indicity)=> indicity.name);

	  setSelectedstate(apnastate[0]);
	  setSelectedcities(citiesarr);
	}

	function fixLandmarks(e)
	{
		let selectedcity=citiesObj.filter((indicity, index)=> {if(index===parseInt(e.target.value)){return indicity}});
		
		let selectedcityobj=selectedcity[0];
        let landmarksarr=selectedcityobj.landmarks;


        setLandobj(landmarksarr);
		let arrofland=landmarksarr.map((plandmark)=> {return plandmark.name});


		setCitytaken(selectedcityobj);
		setSelectedlandmarks(arrofland);
	}

	function tofixselectLand(e)
	{
		
      let parr=landObj.filter((indilandmark, index)=> {if(index===parseInt(e.target.value)){return indilandmark}});
	  setLandmarktaken(parr[0]);
	}
	
	return (
		<>
	<div id="main">
		<select style={{margin: '20px'}} id="state" onChange={(e)=> fixCities(e)}>
			{states.map((pstate, index)=> {return <option key={pstate.name} value={index}>{pstate.name}</option>})}
			</select>

         <select  style={{margin: '20px'}} id="city" onChange={(e)=> fixLandmarks(e)}>
             {selectedcities.map((pcity,index)=> {return <option key={pcity} value={index}>{pcity}</option>})}
		 </select>

		 <select style={{margin: '20px'}} id="landmark" onChange={(e)=> tofixselectLand(e)}>
             {selectedlandmarks.map((partlandmark,index)=> {return <option key={partlandmark} value={index}>{partlandmark}</option>})}
		 </select>
	</div>
	<div id="state-name">
	...
	<div id="city-name">
		 {citytaken && citytaken.name}
	</div>
	<div id="city-description">
	     {citytaken && citytaken.description}
	</div>
	<div id="landmark-name">
		{landmarktaken && landmarktaken.name}
	</div>
	<div id="landmark-description">
	{landmarktaken && landmarktaken.description}
	</div>
	</>
	);
}
